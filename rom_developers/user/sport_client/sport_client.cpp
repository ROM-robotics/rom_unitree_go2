...
public:
  ...
  void HighStateHandler(const void *message)
  {
    ...
    //   └─ message သည် const void* (type မသိသော raw pointer) ဖြစ်သည်။
    //      DDS callback သည် data ကို void* ဖြင့် ပေးပို့သောကြောင့်
    //      စစ်မှန်သော type ဖြစ်သည့် SportModeState_* အဖြစ် cast လုပ်သည်။
    //
    // *(unitree_go::msg::dds_::SportModeState_ *)message
    //   └─ cast ပြုလုပ်ပြီးသော pointer ကို * ဖြင့် dereference လုပ်သည်။
    //      pointer ထဲရှိ address မှ SportModeState_ object တစ်ခုလုံးကို
    //      value copy အဖြစ် state member variable ထဲသို့ သိမ်းသည်။
  ...
  double px0, py0, yaw0; // ကနဦး position (x, y) နှင့် yaw ထောင့်
  double ct = 0;         // program စတင်ကတည်းက ကြာချိန် (seconds)
  int flag = 0;          // တစ်ကြိမ်သာ လုပ်ရမည့် special action ပြီးမပြီး flag
  float dt = 0.005;      // control loop step size (0.001~0.01 seconds)
};
...
int main(int argc, char **argv)
{
  ...
  custom.GetInitState(); // ကနဦး position နှင့် yaw ကို မှတ်သားသည်

  // dt * 1000000 → microseconds ပြောင်းပြီး loop interval သတ်မှတ်သည် (dt=0.005s → 5000us = 5ms တစ်ကြိမ်)
  // RobotControl() function ကို သတ်မှတ်ထားသော interval တိုင်း recurrent thread ဖြင့် ခေါ်သည်
